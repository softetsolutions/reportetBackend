...
export const orgRegister = async (req, res) => {
  ...
  const orgcode = generateOrganizationCode(organizationName);
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Organization registered successfully" });
  } catch (error) {
    console.error("Org Register Error:", error);
    res.status(500).json({
      success: false,
      message: `Cannot register the organization. Reason:-${error.message}`,
    });
  }
};
